...
const roles = [
  {
    ...
    bullets: [
      'Built Java/Spring Boot repayment microservices on Oracle and GCP with Redis caching, processing 10M+ daily transactions across 16 products in 8 countries with exactly-once execution guarantees.',
  ...
  },
  {
    ...
    bullets: [
      'Architected production AI support agent in Python with LangChain, LangGraph, and MCP-based tool calling, integrating telemetry and internal RESTful APIs to automate 30% of repetitive support workflows on a multi-tenant CMMS platform.',
      'Designed RAG pipeline with ChromaDB, OpenAI embeddings, and Pandas/NumPy preprocessing, implementing document chunking and metadata filtering to lift retrieval accuracy by 60% for technician troubleshooting.',
      'Deployed quantized local LLM with PyTorch and Hugging Face Transformers on NVIDIA Jetson Orin Nano, enabling sub-second edge inference for privacy-sensitive operations and removing external AI API dependency.',
      'Cut Azure infrastructure spend by 20% and deployment time by 60% through Terraform based IaC, GitHub Actions optimization, and caching high volume read paths with Redis.',
    ],
  ...
  },
  {
    ...
    bullets: [
      'Launched full-stack customer portal features using Next.js, TypeScript, and GraphQL, reducing client-side API traffic by 30% through typed data fetching and batched backend requests.',
      'Hardened Spring Boot microservices with Spring Security, enforcing OAuth 2.0 authorization with JWT bearer tokens and role-based access control across distributed services.',
      'Consolidated containerized applications on AWS EC2 via Docker, standardizing production and staging environments to eliminate configuration drift in deployments.',
  ...
  },
  {
    ...
    bullets: [
      ...
      'Refactored MySQL schemas with targeted indexing and query optimization, cutting dashboard load times by 50%.',
      'Automated Jenkins and Kubernetes CI/CD pipelines with integrated test suites, shortening release cycle time by 40%.',
    ],
